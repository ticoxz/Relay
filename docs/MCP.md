...
Requires `relay-mcp` on PATH (`pnpm add -g @ticoxz/relay@1.3.1` or `pnpm link --global` from a Git clone).
...
90 days after `@ticoxz/relay@1.3.1` is on the registry (`pnpm publish`): if MCP/JSON are unused, defer new MCP features and focus on readers, `relay doctor`, and HANDOFF.md workflow. See [doc.md](../doc.md) §8.
