...
export const installHooksCommand = new Command('install-hooks')
  ...
  .action(() => {
    try {
      ...
      console.log('  pre-commit    → contextvc sync --quiet + git add sesiones');
      console.log('  post-checkout → contextvc handoff --from-repo --quiet');
      console.log('  post-merge    → contextvc handoff --from-repo --quiet');
    } catch (error: any) {
